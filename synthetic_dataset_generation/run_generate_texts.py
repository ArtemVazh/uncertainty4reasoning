...
def parse_args():
    ...
    parser.add_argument('--final-answers', action=argparse.BooleanOptionalAction, default=False,
...
def load_bon_dataset(dataset_path, dataset_split, hf_cache=None):
    if dataset_path[0] == 'local':
        ...
        dataset = load_dataset(*dataset_path, cache_dir=hf_cache)
        if dataset_split is not None:
            dataset = dataset[dataset_split[0]]
    return dataset
...
def main(args):
    ...
    dataset = load_dataset(args.dataset_path, args.dataset_split, args.hf_cache)
