...
#SBATCH --job-name=extract_claim_qwen3_8b_thinking_%a
#SBATCH --output=extract_claim_qwen3_8b_thinking_%a.out
#SBATCH --error=extract_claim_qwen3_8b_thinking_%a.err
...
ANNOTATION_CMD="PYTHONPATH=${ROOT} python -m synthetic_dataset_generation.run_extract_claims_thinking \
  ...
  --save-path $ROOT/gen_data/train_prm800k_qwen3_8b_thinking_${SAMPLE_SIZE}_extracted_${PARTITION_IDX} \
  --hf-save-path JingweiNi/train_prm800k_qwen3_8b_thinking_${SAMPLE_SIZE}_extracted_${PARTITION_IDX} \
  --start-idx 0 \
  --subset $SAMPLE_SIZE \
