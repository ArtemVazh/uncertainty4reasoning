...
#SBATCH --job-name=phi4-reasoning-plus_annotate_prm_%A_%a
#SBATCH --output=phi4-reasoning-plus_annotate_prm_%A_%a.out
#SBATCH --error=phi4-reasoning-plus_annotate_prm_%A_%a.err
...
ANNOTATOR_MODEL="microsoft/Phi-4-reasoning-plus"
ANNOTATOR_NAME="Phi-4-reasoning-plus"
VLLM_LOG="phi4_reasoning_plus_vllm_${SLURM_ARRAY_TASK_ID}.log"
DEBUG=0
PARTITION_NUM=8
...
ANNOTATION_CMD="PYTHONPATH=${ROOT} python -m synthetic_dataset_generation.run_extract_verify_claims_self \
  ...
  --save-path $ROOT/gen_data/train_prm800k_${ANNOTATOR_NAME}_finished_self_annotate_${PARTITION_IDX} \
  --hf-save-path JingweiNi/train_prm800k_${ANNOTATOR_NAME}_finished_self_annotate_${PARTITION_IDX} \
  --api-cache $ROOT/gen_data/${ANNOTATOR_NAME}_self_annotate_${PARTITION_IDX} \
