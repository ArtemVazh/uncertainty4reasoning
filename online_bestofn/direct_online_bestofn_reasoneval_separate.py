...
def run_separate_evaluations(
    ...
    for criterion, save_path in [("validity", save_path_validity), 
                                  ("redundancy", save_path_redundancy)]:
        ...
        try:
            for i, sample in enumerate(tqdm(dataset, desc=f"Processing samples ({criterion})")):
                if verbose:
                    log.info(f"\n{'='*60}")
        ...
        log.info(f"\n{criterion.capitalize()}-based accuracy: {correct}/{len(results)} = {correct/len(results):.2%}")
