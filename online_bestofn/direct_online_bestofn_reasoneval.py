"""
Direct online best-of-n implementation using direct ReasonEval scoring
"""
...
from online_bestofn.scorers.reasoneval_direct import DirectReasonEvalScorer
from online_bestofn.direct_online_bestofn import _is_correct_answer

log = logging.getLogger(__name__)


class DirectOnlineBestOfNReasonEval:
    """
    Online best-of-n using direct ReasonEval scoring.
    
    Key features:
    1. Direct ReasonEval application - no stat calculator overhead
    2. Uses validity and redundancy scores to compute uncertainty
    3. Step-by-step generation with reasoning quality guidance
    """
    ...
    def __init__(
        ...
        self.scorer = DirectReasonEvalScorer(
            model=model,
    ...
    def generate_trajectory(self, prompt: str) -> Dict[str, any]:
        """
        Generate a trajectory step-by-step using direct ReasonEval scoring.
        ...
        Returns:
            Dictionary with:
                ...
                - step_scores: Uncertainty scores for each selected step
        ...
        for step_num in range(self.max_steps):
            ...
            # Score candidates directly with ReasonEval
            ...
            if self.verbose:
                ...
                for i, (candidate, score) in enumerate(zip(candidates, candidate_scores)):
                    log.info(f"  [{i}] Uncertainty: {score:.3f} | Text: '{candidate.text}'")
            ...
            if self.verbose:
                log.info(f"Selected candidate {best_idx} (uncertainty: {selected_score:.3f})")
    ...
    def _generate_final_answer(self, trajectory: str) -> str:
        ...
        for uncertainties in answer_uncertainties:
            if uncertainties and len(uncertainties) > 0:
                if hasattr(uncertainties, 'mean'):
                    score = float(uncertainties.mean())
                else:
                    score = float(sum(uncertainties) / len(uncertainties))
            else:
                score = 0.5
            answer_scores.append(score)
        
        # Select best answer (lowest uncertainty)
        ...
        if self.verbose:
            ...
            log.info(f"Selected answer {best_idx} (uncertainty: {answer_scores[best_idx]:.3f})")
...
def run_direct_online_bestofn_reasoneval(
    dataset: Dataset,
    model: WhiteboxModel,
    ...
    Args:
        ...
        aggregation: How to aggregate validity/redundancy scores
