#!/usr/bin/env python3
"""
Run direct online best-of-n evaluation with UHead
"""
...
import traceback
from online_bestofn.direct_online_bestofn import run_direct_online_bestofn
...
def get_parser():
    ...
    parser = argparse.ArgumentParser(description="Direct online best-of-n with UHead")
...
def main(args):
    ...
    # Run direct online best-of-n
    log.info(f"Starting direct online best-of-n evaluation")
    ...
    if all_scores:
        ...
        log.info(f"  - Avg step uncertainty: {np.mean(all_scores):.3f}")
        log.info(f"  - Min step uncertainty: {np.min(all_scores):.3f}")
        log.info(f"  - Max step uncertainty: {np.max(all_scores):.3f}")
